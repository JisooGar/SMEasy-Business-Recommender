...
function kMeansClustering(locations, k = 5, iterations = 10) {
    ...
    //If the input array locations is empty, the function exits early with a message and returns empty results (centroids and clusters).
    if (locations.length === 0) {
        console.error("No locations provided for clustering.");
        return { centroids: [], clusters: [] };
    }

//centroids: Stores the central point for each cluster.
//clusters: Stores arrays of locations assigned to each cluster.
    let centroids = [];
    let clusters = [];

    // Select k random locations from locations as the initial centroids.
    ...
    //The algorithm refines clusters and centroids repeatedly, up to the specified number of iterations.
    for (let iter = 0; iter < iterations; iter++) {

        //Create k empty arrays in the clusters array. Each will hold the locations belonging to a specific cluster.
        clusters = Array(k).fill(null).map(() => []);

        // Assign each location to the closest centroid
        locations.forEach(location => {
            let closestCentroid = 0;
            //Measure its distance using calculateDistance.
            ...
            for (let i = 1; i < centroids.length; i++) {
                ...
                if (distance < minDistance) {
                    ...
                    //Update closestCentroid whenever a smaller distance is found.
                    closestCentroid = i;
                }
            }
            //Add the location to the array of the nearest centroid in clusters.
            clusters[closestCentroid].push(location);
        });

        // Recalculate centroids
        centroids = clusters.map(cluster => {
            //For each cluster, calculate the average latitude and longitude of all points in that cluster.
            ...
            //If a cluster is empty, set its centroid to null.
...
// measuring the shortest distance over the Earth's surface between two points (latitude/longitude).
//If point2 is null, it returns Infinity to avoid assigning a location to a null centroid.
